          ...
          {Object.keys(items).map((key, index) => {
            return (
              <div id={index} onClick={() => {
                setState(index)
              }} className={index === state ? "flex justify-center items-center w-1/4 border-t border-l border-r border-black cursor-pointer relative text-[10px] 5md:text-sm " : `flex justify-center items-center w-1/4 height-auto border-t border-r border-l border-b-[1px] border-b-black border-gray-300 px-1 py cursor-pointer text-[10px] 5md:text-sm ${index === state - 1 && "border-r-transparent"} ${index === state + 1 && "flex justify-center items-center w-1/4 height-auto border-l-transparent text-[10px] 5md:text-sm"}`}>
                {key}
              </div>
            )
          }
